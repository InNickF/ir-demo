...
const { numberToDollar, numberToPercent, readableNumber } = amountConversions;
...
const columnHelper = createColumnHelper<TransitionalProperty>();

const useTransactionalPropertiesTable = (
  data: TransitionalProperty[],
  columns: ColumnDef<TransitionalProperty>[]
) => {
...
export const TransactionalPropertiesTable: FC<
  CompTableProps<TransitionalProperty, TransitionalPropertyTotals>
> = ({
  data,
  page,
  totals,
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("price_sf", {
        header: "Price/SF",
        cell: (info) => numberToDollar({ value: info.getValue() }),
      }),
      columnHelper.accessor("cap_rate", {
        header: "Cap Rate",
        cell: (info) => numberToPercent(info.getValue()),
      }),
  ...
  const table = useTransactionalPropertiesTable(data, columns);
  return (
    <>
      <TableCard.Body>
        <Table>
              ...
              <Table.Row>
                ...
                <Table.Data>
                  {isLoadingTotals ? (
                    ...
                    numberToDollar({ value: totals?.price_sf__wavg })
                  )}
                </Table.Data>
                <Table.Data>
                  {isLoadingTotals ? (
                    ...
                    numberToPercent(totals?.cap_rate__wavg)
                  )}
                </Table.Data>
