import { CompstackLandComp } from "@/acquisitions/typings/market-analytics";
import { SlicedText } from "@/commons/components/data-display/SlicedText";
import { compstackLandCompValueFormatters } from "@/modules/acquisitions/utils/formatters/comps-value-formatters";
...
const columnHelper = createColumnHelper<CompstackLandComp>();

export const useLandCompsTable = ({
  data,
  tableActions = null,
}: UseCompstackCompsTableKookParams<CompstackLandComp>) => {
  const columns = useMemo(() => {
    const cols = [
      columnHelper.accessor("sales_date", {
        header: "Sales Date",
        cell: (info) =>
          compstackLandCompValueFormatters.sales_date(info.getValue()),
      }),
      ...
      columnHelper.accessor("in_place_cap_rate", {
        ...
        cell: (info) =>
          compstackLandCompValueFormatters.in_place_cap_rate(info.getValue()),
      }),
      columnHelper.accessor("price_per_sf", {
        header: "Price Per SF",
        cell: (info) =>
          compstackLandCompValueFormatters.price_per_sf(info.getValue()),
      }),
      columnHelper.accessor("sale_price", {
        header: "Sale Price",
        cell: (info) =>
          compstackLandCompValueFormatters.sale_price(info.getValue()),
      }),
      columnHelper.accessor("building_condition", {
        header: "Building Condition",
        cell: (info) =>
          compstackLandCompValueFormatters.building_condition(info.getValue()),
      }),
      columnHelper.accessor("size", {
        header: "Size",
        cell: (info) => compstackLandCompValueFormatters.size(info.getValue()),
      }),
      columnHelper.accessor("hold_period_months", {
        header: "Hold Period Months",
        cell: (info) =>
          compstackLandCompValueFormatters.hold_period_months(info.getValue()),
      }),
      columnHelper.accessor("hold_period_years", {
        header: "Hold Period Years",
        cell: (info) =>
          compstackLandCompValueFormatters.hold_period_years(info.getValue()),
      }),
      columnHelper.accessor("amenities", {
        header: "Amenities",
        cell: (info) =>
          compstackLandCompValueFormatters.amenities(info.getValue()),
      }),
...
export const CompstackLandCompsTable: FC<
  CompstackCompTableProps<CompstackLandComp>
