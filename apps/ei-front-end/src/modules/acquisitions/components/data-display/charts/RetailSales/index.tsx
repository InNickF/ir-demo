import { CardWithHeader } from "@/commons/components/general/CardWithHeader";
import { numberToDollar } from "@/commons/model-in/formatters/utils/amount-conversions";
import { getChartDateFromString } from "@/commons/utils/charts";
import { useHistoricalMacroEconomicIndicatorsChart } from "@/modules/acquisitions/services/queries/market-analytics";
import { ChartBarSquareIcon } from "@heroicons/react/24/outline";
import dynamic from "next/dynamic";
import { FC, useMemo } from "react";
import { NoDataOnChartMessage } from "../../NoDataOnChartMessage";
import { CurrentValueChartHeader } from "../CurrentValueChartHeader";
import { MarketAnalyticsChartProps } from "../types";
import { Loader } from "in-ui-react";
import { Loader3D } from "@/commons/components/3d/Loader3D";

const RetailSalesChart = dynamic(
  () => import("@/commons/components/data-display/charts/GenericDateLineChart"),
  {
    ssr: false,
    loading: () => (
      <div className="flex w-full justify-center items-center h-96">
        <Loader3D
          kind="chart"
          style={{
            minHeight: 350,
          }}
          isLoading
          localIsLoading
          onChangeIsLoading={() => null}
        />
      </div>
    ),
  }
);

export const RetailSales: FC<MarketAnalyticsChartProps> = ({
  headerActions,
  className,
  filters,
  ...props
}) => {
  const isFilteringByMarket = "market" in props;
  const filtersBy = isFilteringByMarket
    ? {
        market: props?.market,
      }
    : {
        submarket: props?.submarket,
      };
  const {
    data = [],
    isLoading,
    isRefetching,
  } = useHistoricalMacroEconomicIndicatorsChart({
    ...filtersBy,
    ...filters,
    type: "retail-sales",
  });
  const formattedData = useMemo(
    () =>
      data
        .map((item) => {
          return getChartDateFromString({
            chartData: item,
            dateKey: "x",
          });
        })
        .sort((a, b) => a.x - b.x),
    [data]
  );
  const hasData = formattedData.length > 0;
  return (
    <CardWithHeader
      title="Retail Sales"
      className={className}
      icon={<ChartBarSquareIcon />}
      headerActions={
        <CurrentValueChartHeader
          value={
            hasData
              ? numberToDollar({
                  value: formattedData[formattedData.length - 1].y,
                  options: {
                    maximumFractionDigits: 0,
                    minimumFractionDigits: 0,
                  },
                })
              : null
          }
          headerActions={headerActions}
        />
      }
      isLoading={isLoading}
      isRefetching={isRefetching}
    >
      <div>
        {hasData ? (
          <RetailSalesChart id="acq-retail-sales-chart" data={formattedData} />
        ) : (
          <NoDataOnChartMessage />
        )}
      </div>
    </CardWithHeader>
  );
};
