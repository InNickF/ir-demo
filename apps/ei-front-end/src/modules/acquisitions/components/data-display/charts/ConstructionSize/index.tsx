...
const ConstructionSizeChart = dynamic(
  () =>
    import(
      "@/acquisitions/components/data-display/charts/GenericPercentageLineChart"
    ),
  {
    ssr: false,
    loading: () => (
...
export const ConstructionSize: FC<MarketAnalyticsChartProps> = ({
  headerActions,
  filters,
  ...props
}) => {
  ...
  return (
    <CardWithHeader
      ...
      headerActions={
        <CurrentValueChartHeader
          value={
            hasData
              ? numberToPercent(formattedData[formattedData.length - 1].y, true)
              : null
          }
    ...
    >
      <div>
        {hasData ? (
          <ConstructionSizeChart
            id="acq-construction-size-chart"
            data={formattedData}
            scaleValues
          />
        ) : (
